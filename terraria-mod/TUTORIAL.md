...
namespace HelloMod
{
    ...
    {
        // Tato třída je vstupní bod modu.
        // Metody Load() a Unload() jsou volány při načtení/vypnutí modu.
        // Pro základní mody stačí prázdná třída – tModLoader vše ostatní
        // načte automaticky ze složek Items/, NPCs/, atd.
    }
}
```
...
namespace HelloMod.Items
{
    ...
    {
        // SetDefaults – nastavení vlastností předmětu
        public override void SetDefaults()
        {
            Item.damage = 75;                   // poškození
            Item.DamageType = DamageClass.Melee; // typ poškození (melee/ranged/magic)
            Item.width = 40;                    // šířka hitboxu
            Item.height = 40;                   // výška hitboxu
            Item.useTime = 20;                  // rychlost použití
            Item.useAnimation = 20;
            Item.useStyle = ItemUseStyleID.Swing; // animace – mávnutí
            Item.knockBack = 6f;                // odhazování nepřátel
            Item.value = Item.buyPrice(gold: 5); // cena v obchodě (5 zlatých)
            Item.rare = ItemRarityID.Orange;    // vzácnost (barva názvu)
            Item.autoReuse = true;              // automatické opakování
            Item.UseSound = SoundID.Item1;      // zvuk při použití
        }

        // SetStaticDefaults – statické informace (tooltip, jméno)
        public override void SetStaticDefaults()
        {
            // Tooltip se zobrazí v inventáři pod jménem předmětu
            Tooltip.SetDefault("Meč ukovaný ze síly slunce\nHoří nepřátelé při zásahu");
        }
...
namespace HelloMod.Projectiles
{
    public class OhnivaKoule : ModProjectile
    {
        public override void SetDefaults()
        {
            ...
            Projectile.aiStyle = ProjAIStyleID.Arrow; // chování jako šíp
...
namespace HelloMod.NPCs
{
    public class PriatelskyCestovatel : ModNPC
    {
        public override void SetDefaults()
        {
            NPC.width = 18;
            NPC.height = 40;
            NPC.friendly = true;   // přátelské NPC
            NPC.lifeMax = 250;
            NPC.HitSound = SoundID.NPCHit1;
            NPC.DeathSound = SoundID.NPCDeath1;
            NPC.knockBackResist = 0.5f;
            NPC.aiStyle = NPCAIStyleID.Passive; // pasivní AI – chodí sem tam
        }

        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Přátelský Cestovatel");
            // Počet snímků animace
            Main.npcFrameCount[NPC.type] = 25;
        }

        // Každý den má šanci se spawnnout ve světě
